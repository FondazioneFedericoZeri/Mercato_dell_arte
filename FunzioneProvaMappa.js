am5.ready(function() {
    ...
    //voglio prendere i dati dal mio json
    // INIZIO SCRIPTS JAVASCRIPT

document.addEventListener("DOMContentLoaded", function() {
    // Fetch data from luoghi.json
    fetch('luoghi.json')
    .then(response => response.json())
    .then(data => {
        // Populate the cities array with data from the JSON file
        var cities = [];

        // Iterate through the JSON object and push the necessary data into cities array
        Object.keys(data).forEach(key => {
            let cityData = data[key];
            cities.push({
                title: cityData.Città,
                latitude: cityData.geo.lat,
                longitude: cityData.geo.lon
            });
        });

        // Now add the cities to the map or use them as needed
        cities.forEach(city => {
