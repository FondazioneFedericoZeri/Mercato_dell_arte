...
def build_places(input_csv, output_json):
	...
	with open(input_csv, encoding="utf-8") as csv_fin:
		...
		for row in tqdm.tqdm(reader):
			...
			location = geolocator.geocode(f"{place['Via']} {place['Civico']} {place['Città']} {place['Nazione']}", timeout=None)
	...
	for vendita in compravendite:
		...
		if ent in entities_dicts:
			...
			if "ID_persone" in compravendite[vendita]:
				entities_dicts[ent]["Clienti"][compravendite[vendita]["ID_cliente"]]["vendite"].append(compravendite[vendita]["ID_persone"])
			else:
				entities_dicts[ent]["Clienti"][compravendite[vendita]["ID_cliente"]]["vendite"].append("Generic")
...
if __name__ == "__main__":
	...
	if sys.argv[1] == "bibliografia":
		build_generic("data/bibliografiaGenerale.tsv", "json/bibliografia.json")

	if sys.argv[1] == "collaboratori":
		build_generic("data/collaboratori.tsv", "json/collaboratori.json")
	...
	if sys.argv[1] == "compravendite":
		build_generic("data/compravendite.tsv", "json/compravendite.json")
