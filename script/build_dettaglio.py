...
def build_html_head(page):
	with page.head():
		...
		page.link(rel="stylesheet", href="../../css/styles-dettaglioAntiquario.css")
		...
		page.script(type="text/javascript", src="../../script/dettaglioAntiquari.js")
		page.script(type="text/javascript", src="https://cdn.jsdelivr.net/npm/jquery@3.2.1/dist/jquery.min.js")
		page.script(type="text/javascript", src="https://cdn.amcharts.com/lib/5/index.js")
		page.script(type="text/javascript", src="https://cdn.amcharts.com/lib/5/map.js")
		page.script(type="text/javascript", src="https://cdn.amcharts.com/lib/5/geodata/worldLow.js")
		page.script(type="text/javascript", src="https://cdn.amcharts.com/lib/5/themes/Animated.js")

		page.script(type="text/javascript", src="../../script/mappaDettaglio.js")
...
def build_html(entity, entities):
	...
	images_description = json.loads(open("json/didascalie.json").read())
	people = json.loads(open("json/persone.json").read())
		...
		with page.body():
			...
			with page.main():
				...
				if len(imgs) == 0:
					with page.section(id="single-image-section"):
						page.h2(_t=f"{entity['Nome']}", klass="single-image-title")
					...
					with page.section(id="image-gallery"):
						...
						with page.div(klass="slider-container"):
							...
							page.button("&#10094;", klass="prev", onclick="prevSlide()")
							page.button("&#10095;", klass="next", onclick="nextSlide()")
			...
			with page.section(id="FZ", klass="fade-in"):
				...
				with page.div(klass="content-wrapper"):
					...
					page.a(href=f"{entity['Link Zeri']}", klass="btn", _t="Clicca qui")
				...
				with page.div(klass="dual-content-wrapper"):
					with page.div(klass="tab-container"):
						with page.ul(klass="tab-list"):
							page.li(_t="Biografia", klass="tab active", data_content="Bio")
							page.li(_t="Albero familiare", klass="tab", data_content="Persone")
							page.li(_t="Mappa dei luoghi", klass="tab", data_content="Localizzazioni")
							if len(entity["Collaboratori"])>0:
								page.li(_t="Collaboratori", klass="tab", data_content="Collaboratori")
							if len(entity["Clienti"])>0:
								page.li(_t="Clienti", klass="tab", data_content="Clienti")
							if len(entity["Eventi"])>0:
								page.li(_t="Eventi", klass="tab", data_content="Eventi")
							if len(entity["Relazioni"])>0:
								page.li(_t="Altri antiquari", klass="tab", data_content="Relazioni")
							if len(entity["Bibliografia"]) > 0:
								page.li(_t="Bibliografia", klass="tab", data_content="Bibliografia")
					...
					with page.div(klass="content-card"):
						...
						with page.div(id="Persone", klass="content"):
							page.h3(_t=f"L'entità {entity['Nome']} è così composta:")
							with page.div(id="albero-genealogico"):
								page.p(_t="nome1 - nome2 - nome3")

						with page.div(id="Localizzazioni", klass="content"):
							page.h3(_t="Geolocalizzazione dell'attività antiquariale:")
						...
						if len(entity["Eventi"])>0:
							with page.div(id="Eventi", klass="content"):
								page.h3(_t="Gli eventi signficativi dell'attività antiquariale:")
								with page.ul():
						...
						if len(entity["Relazioni"])>0:
							with page.div(id="Relazioni", klass="content"):
								page.h3(_t=f"L'entità {entity['Nome']}, durante la sua attività, ha avuto relazioni con:")
								with page.ul():
									for relent_id in entity['Relazioni']:
										with page.li():
											page.a(href=f"dettaglio_{relent_id}.html", _t=f"{entities[relent_id]['Nome']}")
	...
	with open(f"html/dettagli/dettaglio_{entity['ID']}.html", "w") as f:
		f.write(html_content)

if __name__ == "__main__":
	entities = json.loads(open("json/entità.json").read())
